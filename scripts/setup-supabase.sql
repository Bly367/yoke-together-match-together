-- Yoke Dating App - Database Schema
...
-- Note: If PostGIS is not available, comment out this line and the location POINT column
...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  bio TEXT,
  photo_url TEXT,
  location POINT, -- PostGIS point for geolocation (comment out if PostGIS not available)
...
DROP POLICY IF EXISTS "Users can update own duos" ON public.duos;
DROP POLICY IF EXISTS "Users can create duos they're a member of" ON public.duos;
DROP POLICY IF EXISTS "Users can view swipes from own duos" ON public.swipes;
DROP POLICY IF EXISTS "Users can create swipes for own duos" ON public.swipes;
DROP POLICY IF EXISTS "Users can view matches for own duos" ON public.matches;
DROP POLICY IF EXISTS "Users can view messages for own matches" ON public.messages;
...
DROP TRIGGER IF EXISTS create_match_on_mutual_like ON public.swipes;
...
DROP TRIGGER IF EXISTS update_duos_updated_at ON public.duos;
